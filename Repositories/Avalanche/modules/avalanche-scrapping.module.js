...
const getTransactionsList = async (params) => {
	try {
		...
		const url1 = `https://www.oklink.com/api/explorer/v2/avaxc/addresses/${id}/transactionsByClassfy/condition?offset=${
			page || "0"
		}&limit=${show || "100"}&address=${id}&nonzeroValue=false&t=${t}`;

		const url2 = `https://www.oklink.com/api/explorer/v2/avaxc/addresses/${id}/transfers/condition/token?t=${t}`;

		// Configura los headers una vez
		const headers = {
		...
		// Ejecuta ambas peticiones en paralelo para mayor rendimiento
		const [res1, res2] = await Promise.allSettled([
			axios.get(url1, { httpsAgent: agent, headers }),
			axios.get(url2, { httpsAgent: agent, headers }),
		]);

		// Extrae los resultados de cada petición si fueron exitosas
		const transfer =
			res1.status === "fulfilled" ? res1.value.data.data?.hits || [] : [];
		const tokenERC20 =
			res2.status === "fulfilled" ? res2.value.data.data?.hits || [] : [];

		// Combina ambos arrays
		const combined = [...transfer, ...tokenERC20];

		// Si no hay transacciones, devuelve array vacío
		if (combined.length === 0) return [];

		// Formatea todas las transacciones combinadas
		const transactions = combined.map((tx) => ({
			...
			date: tx.blocktime ? moment(tx.blocktime * 1000) : "",
			from: tx.from || "",
			traffic: tx.realValue < 0 ? "OUT" : "IN",
			to: tx.to || "",
			amount: typeof tx.value === "number" ? tx.value.toFixed(4) : "0.0000",
			asset: "AVAX",
			txnFee: tx.fee ? tx.fee.toFixed(4) : "0.0000",
		}));
...
const getTransactionDetail = async (params) => {
	...
	try {
		...
		const transactionFeeFiat = $("#data-tfprice")
			.text()
			.replace("$", "")
			.replace(")", "")
			.replace("(", "")
