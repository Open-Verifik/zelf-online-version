...
const _sendInvitationEmail = async ({ staffEmail, staffName, ownerName, company, inviteLink, role }) => {
	try {
		...
		if (!emailResult) {
			console.error("Failed to send invitation email to:", staffEmail);
		} else {
			console.log("Invitation email sent successfully to:", staffEmail);
		}
	} catch (emailError) {
...
const createFromInvitation = async (data) => {
	...
	try {
		...
		if (!emailResult) {
			console.error("Failed to send welcome email to:", invitation.staffEmail);
		} else {
			console.log("Welcome email sent successfully to:", invitation.staffEmail);
		}
	} catch (emailError) {
