...
/**
 * @title ZelfBasePay
 * @notice Accepts native ETH on Base or USDC / USDT (ERC-20), forwards to a fixed treasury. Emits Paid for off-chain verification.
 * @dev Paid.amount is wei (18 decimals) for pay(), and stablecoin base units per token decimals for payUsdc / payUsdt.
 */
contract ZelfBasePay {
    ...
    /**
     * @param paymentId Server-issued binding (keccak256 over session data), must match JWT smartContractBASE.paymentId
