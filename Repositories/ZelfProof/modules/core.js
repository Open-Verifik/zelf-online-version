...
const encrypt = async (data) => {
	try {
		const encryptedResponse = await axios.post("/core/encrypt/zelfProof", {
...
const encryptQR = async (data) => {
	try {
		const encryptedResponse = await axios.post(
			"/core/encrypt/zelfProof-qr-code",
			{
				cleartext_data: data.cleartext_data || data.publicData,
				face_base_64: data.faceBase64,
				metadata: data.metadata,
...
const decrypt = async (data) => {
	...
	try {
		const encryptedResponse = await axios.post("/core/decrypt/zelfProof", {
			face_base_64: data.faceBase64,
			os: data.os || "DESKTOP",
			password: data.password || undefined,
...
const preview = async (data) => {
	try {
		const encryptedResponse = await axios.post("/core/preview/zelfProof", {
			senseprint_base_64: data.zelfProof,
