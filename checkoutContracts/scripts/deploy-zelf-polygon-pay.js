#!/usr/bin/env node
/**
 * Deploy ZelfPolygonPay: compiles with `npx solc@0.8.20`, deploys with ethers.
 *
 * Uses the same deployer secret as Avalanche / BSC / Ethereum tag pay:
 *   - AVALANCHE_PRIVATE_KEY — hex key or BIP39 mnemonic
 *
 * Treasury defaults to the deployer wallet address (same EVM identity on Polygon as on other chains).
 *
 * Optional:
 *   - POLYGON_RPC_URL
 *   - POLYGON_CHAIN_ID (137 Polygon mainnet, 80002 Amoy testnet)
 *   - ZELF_CHECKOUT_TREASURY — override treasury address
 *   - POLYGON_TAG_PAY_USDC_ADDRESS — USDC on Polygon
 *   - POLYGON_TAG_PAY_USDT_ADDRESS — USDT on Polygon
 *   - AVALANCHE_HD_PATH — same as other deploys when using mnemonic
 *
 * Usage (from zelf repo root):
 *   npm run deploy:checkout-polygon
 *
 * Compile only:
 *   DRY_RUN=1 npm run deploy:checkout-polygon
 */
...
const DEFAULT_RPC_MAINNET = "https://polygon-rpc.com";
const DEFAULT_RPC_AMOY = "https://rpc-amoy.polygon.technology";
/** USDC (PoS) on Polygon mainnet (6 decimals) */
const DEFAULT_USDC_MAINNET = "0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174";
/** USDT on Polygon mainnet (6 decimals) */
const DEFAULT_USDT_MAINNET = "0xc2132D05D31c914a87C6611C10748AEb04B58e8F";
...
function compile() {
    const solFile = path.join(checkoutRoot, "src", "ZelfPolygonPay.sol");
    if (!fs.existsSync(solFile)) {
        throw new Error(`Missing ${solFile}`);
    }

    const outDir = path.join(checkoutRoot, ".solc-out-polygon");
    ...
    if (!bins.length) {
        throw new Error("solc produced no .bin file under .solc-out-polygon");
    }

    const preferred =
        bins.find((f) => /ZelfPolygonPay.*ZelfPolygonPay\.bin$/i.test(f)) ||
        bins.find((f) => f.includes("ZelfPolygonPay")) ||
        [...bins].sort(
            (a, b) =>
...
async function main() {
    ...
    console.log("Compiling ZelfPolygonPay.sol (solc@0.8.20)…");
    ...
    if (!normalizeSecret(process.env.AVALANCHE_PRIVATE_KEY)) {
        console.error("Missing AVALANCHE_PRIVATE_KEY in .env (zelf repo root). Same key as Avalanche / BSC / ETH tag pay deploy.");
        process.exit(1);
    }

    const chainId = Number(process.env.POLYGON_CHAIN_ID || 137);
    const rpcUrl =
        process.env.POLYGON_RPC_URL || (chainId === 80002 ? DEFAULT_RPC_AMOY : DEFAULT_RPC_MAINNET);
    ...
    const usdcEnv = (process.env.POLYGON_TAG_PAY_USDC_ADDRESS || "").trim();
    const usdtEnv = (process.env.POLYGON_TAG_PAY_USDT_ADDRESS || "").trim();
    const isMainnet = chainId === 137;

    if (!isMainnet && chainId === 80002 && (!usdcEnv || !usdtEnv)) {
        console.error(
            "Amoy (80002): set POLYGON_TAG_PAY_USDC_ADDRESS and POLYGON_TAG_PAY_USDT_ADDRESS in .env (no baked defaults for testnet stables).",
        );
        process.exit(1);
    }
    ...
    if (!usdcRaw || !usdtRaw) {
        console.error("Set POLYGON_TAG_PAY_USDC_ADDRESS and POLYGON_TAG_PAY_USDT_ADDRESS for this network.");
        process.exit(1);
    }
    ...
    console.log("\nDeployed ZelfPolygonPay at:", address);
    console.log("\nSet on the API server:");
    console.log(`  POLYGON_TAG_PAY_CONTRACT_ADDRESS=${address}`);
    console.log(`  POLYGON_TAG_PAY_USDC_ADDRESS=${usdc}`);
    console.log(`  POLYGON_TAG_PAY_USDT_ADDRESS=${usdt}`);
    console.log(`  POLYGON_CHAIN_ID=${chainId}`);
    console.log(`  POLYGON_RPC_URL=<same RPC you used>`);
}
