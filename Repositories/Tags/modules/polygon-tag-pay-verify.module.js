 ...
 */
function classifyPolygonTagPayMode(tx) {
    try {
...
/**
 * Polygon: verify ZelfPolygonPay tx receipt + Paid event vs JWT smartContractPOLYGON slice.
 */
async function verifyPolygonZelfTagPayTx({
    normalizedHash,
    expectedContract,
    chainId,
    rpcUrl,
    ...
    const payMode = classifyPolygonTagPayMode(tx);
    ...
    if (payMode === "NATIVE") {
        amountToPay = prices?.POL?.amountToSend;
        if (amountToPay == null) throw new Error("409:pol_price_missing");
        expectedAtomic = BigInt(sc.expectedWei);
        amountReceivedHuman = Number(eventAmount) / 1e18;
    } else if (payMode === "USDC") {
        amountToPay = prices?.POL_USDC?.amountToSend;
        if (amountToPay == null) throw new Error("409:pol_usdc_price_missing");
        ...
        amountToPay = prices?.POL_USDT?.amountToSend;
        if (amountToPay == null) throw new Error("409:pol_usdt_price_missing");
