...
// Initialize Polygon formatter
const polygonFormatter = new StandardizedChainFormatter("Polygon", "MATIC", "https://s2.coinmarketcap.com/static/img/coins/64x64/3890.png");
...
const getAddress = async (query) => {
	try {
		...
		const timeoutPromise = new Promise((_, reject) => {
			setTimeout(() => reject(new Error("Polygon API timeout after 8 seconds")), 8000);
		});
		...
		const dataPromise = (async () => {
			...
			const nativeMaticToken = {
				...
				name: "Polygon",
				price: maticPrice,
				amount: parseFloat(maticBalance),
			...
			tokens.unshift(nativeMaticToken);
			...
			const formattedResponse = polygonFormatter.formatAddressData(rawData, address, false);

			// Validate the response
			if (!polygonFormatter.validateResponse(formattedResponse)) {
				console.warn("Polygon response validation failed, returning empty response");
				return polygonFormatter.getEmptyResponse(address).data;
			}
		...
		console.error("Polygon getAddress error:", error.message || "Unknown error");
		const errorResponse = polygonFormatter.getErrorResponse(error.message);
...
const getTokens = async (params, query) => {
	try {
		...
		const url = `https://www.oklink.com/api/explorer/v2/polygon/addresses/${address}/tokens?offset=0&limit=${query.show || "100"}&t=${t}`;
		...
		if ((data.code === "0" || data.code === 0) && data.data && data.data.hits) {
			const rawTokens = data.data.hits.map((token) => ({
				...
				image: token.logo || "https://s2.coinmarketcap.com/static/img/coins/64x64/3890.png",
				tokenType: "ERC20",
				owner: address,
			...
			const tokenHoldings = polygonFormatter.formatTokenHoldings(rawTokens);
		...
		console.error("Error getting Polygon tokens:", error.message || "Unknown error");
		return {
			balance: "0",
			total: 0,
			tokens: [],
...
const getTransactionsList = async (query) => {
	try {
		...
		// Get MATIC price for transaction formatting
		let price = "1.00000000";
		try {
			const priceData = await getTickerPrice({ symbol: "MATIC" });
			price = priceData.price || "1.00000000";
		} catch (priceError) {
		...
		// Try PolygonScan API as fallback
		if (!response) {
			try {
				console.log("Trying PolygonScan API...");
				const polygonScanResponse = await getPolygonScanTransactions(address, page, show, price);
				if (polygonScanResponse && polygonScanResponse.transactions && polygonScanResponse.transactions.length > 0) {
					response = polygonScanResponse;
					source = "polygonscan";
					console.log(`Found ${polygonScanResponse.transactions.length} transactions via PolygonScan`);
				}
			} catch (polygonScanError) {
				console.log("PolygonScan failed:", polygonScanError.message);
			}
		}
		...
		console.error("Error getting Polygon transactions:", error.message || "Unknown error");
		return { pagination: { records: "0", pages: "0", page: "0" }, transactions: [] };
	}
};

/**
 * Get OKLink transactions for Polygon
 */
		...
		if ((data.code === "0" || data.code === 0) && data.data && data.data.hits) {
			...
			const rawTransactions = data.data.hits.map((tx) => {
				...
				return {
					...
					traffic: traffic,
					method: tx.method || "MATIC transfer",
...
/**
 * Get PolygonScan transactions
 */
const getPolygonScanTransactions = async (address, page, show, price) => {
	try {
		const url = `https://api.polygonscan.com/api?module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&page=${page}&offset=${show}&sort=desc`;
		...
		if (data.status === "1" && data.result && data.result.length > 0) {
			const rawTransactions = data.result.map((tx) => {
				...
				return {
					...
					traffic: traffic,
					method: tx.methodId ? "Contract Interaction" : "MATIC transfer",
			...
			const transactions = polygonFormatter.formatTransactions(rawTransactions);
		...
		console.log("PolygonScan transactions failed:", error.message);
		return null;
	}
};
...
const getTransactionStatus = async (params) => {
	try {
		...
		const t = Date.now();
		const url = `https://www.oklink.com/api/explorer/v1/polygon/transactions/${id}?t=${t}`;
		...
		console.error("Error getting Polygon transaction status:", error.message || "Unknown error");
...
const getGasTracker = async (query) => {
	try {
		...
		console.error("Error getting Polygon gas tracker:", error.message || "Unknown error");
		return { error: "Failed to fetch gas tracker data" };
	}
};

/**
 * Get portfolio summary for a Polygon address
 */
const getPortfolioSummary = async (params) => {
	try {
		const { address } = params;

		// Get MATIC balance and price
		let maticBalance = "0";
		let maticPrice = "0";
		try {
			const balanceResponse = await instance.post(
			...
			console.log("Portfolio summary MATIC data fetch failed:", error.message);
		}
		...
		// Calculate total portfolio value
		const maticValue = parseFloat(maticBalance) * parseFloat(maticPrice);
		const totalPortfolioValue = maticValue + totalTokenValue;

		return {
			address,
			totalPortfolioValue: totalPortfolioValue.toFixed(2),
			maticBalance: parseFloat(maticBalance).toFixed(8),
			maticValue: maticValue.toFixed(2),
		...
		console.error("Error getting Polygon portfolio summary:", error.message || "Unknown error");
