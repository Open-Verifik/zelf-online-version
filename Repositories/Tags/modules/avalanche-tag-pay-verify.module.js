 ...
 * @returns {"NATIVE" | "USDC" | null}
 */
function classifyTagPayMode(tx) {
    try {
...
/**
 * Avalanche C-Chain: verify ZelfAvalanchePay tx receipt + Paid event vs JWT smartContractAVAX slice.
 * @param {object} params
 * @param {string} params.normalizedHash
 * @param {string} params.expectedContract - checksummed contract from config
 * @param {number} params.chainId
 * @param {string} params.rpcUrl
 * @param {number} params.confirmations
 * @param {object} params.sc - tokenDecoded.smartContractAVAX
 * @param {boolean} params.hasNativeWei
 * @param {boolean} params.hasUsdcPayload
 * @param {string} params.tagNameFull - tokenDecoded.tagName
 * @param {object} [params.prices] - tokenDecoded.prices (AVAX / USDC amountToSend)
 * @param {string} [params.tagPayUsdcAddress] - optional configured USDC token (trimmed or empty)
 * @returns {Promise<{ ok: true, payMode: string, eventAmount: bigint, amountReceivedHuman: number, normalizedHash: string, amountToPay: * } | { ok: false, body: object }>}
 */
async function verifyAvalancheZelfTagPayTx({
    normalizedHash,
    expectedContract,
    chainId,
    rpcUrl,
    ...
    if (!receipt || Number(receipt.status) !== 1) {
        ...
        if (hasNativeWei) {
            payMode = "NATIVE";
            amountToPay = prices?.AVAX?.amountToSend;
    ...
    const payMode = classifyTagPayMode(tx);
...
module.exports = {
    normalizeTagPayTxHash,
    verifyAvalancheZelfTagPayTx,
};
