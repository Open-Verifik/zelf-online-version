...
const mockQrCodeReader = jest.fn();

jest.mock("canvas", () => mockCanvas);
jest.mock("jsqr", () => mockJsQR);
jest.mock("qrcode-reader", () => mockQrCodeReader);
...
describe("QRZelfProofExtractor", () => {
	...
	beforeEach(() => {
		...
		mockCanvas.createCanvas.mockReset();
		mockCanvas.loadImage.mockReset();
		mockJsQR.mockReset();
		mockQrCodeReader.mockReset();
	});
	...
	describe("extractZelfProof", () => {
		it("should extract ZelfProof from base64 PNG image", async () => {
			...
			mockCanvas.createCanvas.mockReturnValue(mockCanvasInstance);
			...
			expect(mockCanvas.loadImage).toHaveBeenCalledWith(expect.any(Buffer));
		...
		it("should return null when canvas is not available", async () => {
			...
			mockCanvas.createCanvas = null;
			mockCanvas.loadImage = null;
		...
		it("should handle errors gracefully", async () => {
			mockCanvas.loadImage.mockRejectedValue(new Error("Image loading failed"));
	...
	describe("_isBase64", () => {
		...
		it("should return false for empty string", () => {
			...
			// But our implementation should return false for empty strings
	...
	describe("Error handling and edge cases", () => {
		it("should handle malformed base64 input", async () => {
			...
			mockCanvas.loadImage.mockRejectedValue(new Error("Invalid image data"));
		...
		it("should handle very large base64 input", async () => {
			...
			mockCanvas.loadImage.mockRejectedValue(new Error("Image too large"));
